...
Calendar.dayMouseDown = function(ev) {
	...
	if (el.navtype != 300) with (Calendar) {
		...
		util.addClass(el, "hilite active");
...
Calendar.dayMouseDblClick = function(ev) {
	...
	if (Calendar.is_ie) {
		document.selection.empty();      //todo check
	}
};
...
Calendar.dayMouseOut = function(ev) {
	with (Calendar) {
		...
		if (isRelated(el, ev) || _C || el.disabled)
			return false;
		util.removeClass(el, "hilite");
		if (el.caldate)
			util.removeClass(el.parentNode, "rowhilite");
		if (el.calendar)
...
{
//    util.writeToLog("Calendar.prototype.setDate=="+date);
...
Calendar.disableCalender = function(el) {
    //var calender=el.calendar;
    el.disabled=true;
};


Calendar.enableCalender = function(el) {
    //var calender=el.calendar;
    el.disabled=false;             
};
...
//       var sDlgId =  "clCalPickerDlg"+sStartDateId
        ...
        if(tfStartDate != null && tfEndDate!=null)
        {
            ...
            {
                var sCurrentDate = clAppBuffer.getTodaysDate(),arrActions=[];
                arrActions[0] = MsgHandler.getMsg(MsgConstants.DONE)+"|clCalendarPicker.onClkGetDates";//3rd param onwards===attributes
                var sUrl = con.getStruts2Url("","calendarPickerDlg.do",null,"calendarPickerDlg");

                sUrl+="sStartLabel="+sStLabel+"&sEndLabel="+sEndLabel+
                      "&sStartDate="+(util.trim(tfStartDate.value).length>0?tfStartDate.value:sCurrentDate)+
                      "&sEndDate="+(util.trim(tfEndDate.value).length>0?tfEndDate.value:sCurrentDate)+
                      "&sEndDateId="+sEndDateId +
                      "&sStartDateId="+sStartDateId +
                      "&isShowTime="+isShowTime+
                      "&iDateFormat="+util.encodeURI(tfStartDate.getAttribute("format"));

                clCalPickerDlg = new DialogBox("clCalPickerDlg",this.sDlgTitle,140,50,460,300,sUrl, false, null, false,true,arrActions,null,null,null,null,false,null,null,true);
            }
            /*else
            {
    ...
    {
        document.getElementById(me.sStartDateId).value = document.getElementById("tfStartDtStr").value;
        document.getElementById(me.sEndDateId).value = document.getElementById("tfEndDtStr").value;
...
//    util.writeToLog("Date.prototype.equalsTo:date=="+date)
...
//    util.writeToLog("Date.prototype.print=="+str)
	var m = this.getMonth();
	var d = this.getDate();
